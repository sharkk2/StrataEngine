...
void main() {
    ...
    float night = clamp(-sunHeight * 5.0, 0.0, 1.0);

    if (useDayNightCycle == 1) {

        float horizonBand = 1.0 - smoothstep(0.05, 0.18, abs(sunHeight));
        float sunsetBlend = clamp(horizonBand, 0.0, 1.0);
        ...
        vec3 sunsetMid = vec3(0.55, 0.22, 0.08);
        vec3 sunsetHorizon = vec3(1.0, 0.42, 0.12);
        vec3 sunsetHaze = vec3(1.0, 0.35, 0.12);

        vec3 nightZenith = vec3(0.001, 0.001, 0.005);
        vec3 nightMid = vec3(0.001, 0.001, 0.004);
        vec3 nightHorizon = vec3(0.002, 0.002, 0.007);
        vec3 nightHaze = vec3(0.001, 0.001, 0.004);

        zenithColor  = mix(nightZenith,  dayZenith,sunUp);
        midColor = mix(nightMid, dayMid, sunUp);
        horizonColor = mix(nightHorizon, dayHorizon, sunUp);
        hazeColor = mix(nightHaze, dayHaze, sunUp);

        float ss = sunsetBlend;
        zenithColor  = mix(zenithColor, sunsetZenith, ss * 0.05);
        midColor  = mix(midColor, sunsetMid, ss * 0.08);
        horizonColor = mix(horizonColor, sunsetHorizon, ss * 0.25);
        hazeColor = mix(hazeColor, sunsetHaze, ss * 0.35);
    ...
    vec3 sunDiscColor = mix(vec3(1.0, 0.4, 0.1), vec3(1.0, 0.97, 0.88), clamp(sunHeight * 3.0, 0.0, 1.0));
    ...
    sunContrib += vec3(1.0, 0.75, 0.45) * mie;
    sunContrib += vec3(1.0, 0.85, 0.55) * outerGlow;
    sunContrib += vec3(8.0, 7.0, 5.5) * innerCorona;
