...
uniform float normalShadowBias = 0.02;
...
float computeShadow(sampler2DShadow map, vec4 posLightSpace, vec3 norm, vec3 lightDir) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        shadow += texture(map, vec3(projCoords.xy + offset, projCoords.z));
    }
    return shadow / 16.0;
}
...
float computeCubeShadow(samplerCubeShadow map, vec3 fragToLight, float farPlane, vec3 norm, vec3 lightDir) {
    ...
    float angle = rand(vec4(gl_FragCoord.xyy, 2.0)) * 6.28318;
    float s = sin(angle);
    float c = cos(angle);
    mat2 rotation = mat2(c, s, -s, c);

    float faceRes = float(textureSize(map, 0).x);
    float texelWorldSize = 2.0 * localZ / faceRes;
    float diskRadius = texelWorldSize * 1.5;

    float shadow = 0.0;
    for (int i = 0; i < 16; i++) {
        vec2 offset = rotation * poissonDisk[i] * diskRadius;
        vec3 sampleDir = fragToLight + tangent * offset.x + bitangent * offset.y;
        shadow += texture(map, vec4(sampleDir, refDepth));
    }
    return shadow / 16.0;
}
...
void main() {
    ...
    if (dlEnabled == 1) {
        ...
        if (globalShadowEnabled == 1) {
            vec4 posLightSpace = globalLightSpaceMatrix * vec4(vWorldPos + norm * normalShadowBias, 1.0);
    ...
    for (int i = 0; i < lightCount; i++) {
        ...
        if (light.shadowSlot >= 0) {
            if (light.type == 1) {
                vec4 posLightSpace = light.lightSpaceMatrix * vec4(vWorldPos + norm * normalShadowBias, 1.0);
                localShadowFactor = computeShadow(shadowTextures[light.shadowSlot], posLightSpace, norm, L);
            } else {
                vec3 fragToLight = (vWorldPos + norm * normalShadowBias) - light.position;
